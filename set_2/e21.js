...
// Middleware to serve static files from the 'e21public' directory.
app.use(express.static(path.join(__dirname,'e21public')));
...
app.get("/login",(req,res)=>{
    ...
    res.sendFile((path.join(__dirname,'e21public','e21login.html')));
});
...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (user) {
            ...
            <body>
              <h1>Welcome To MAYA BAZAAR, ${username}!</h1>
...
app.get("/new", (req, res) => {
    ...
    res.sendFile((path.join(__dirname, 'e21public', 'e21newlogin.html')));
});
...
app.get('/create', (req, res) => {
    ...
    res.sendFile(path.join(__dirname,'e21public','e21.html'));
});
...
app.post('/api/create', async (req, res) => {
    ...
    try {
        ...
        res.redirect(`/e21c.html?id=${details.id}&name=${encodeURIComponent(details.name)}&model=${encodeURIComponent(details.model)}&price=${details.price}&image=${encodeURIComponent(details.image)}&type=create`);
...
app.get('/list', (req, res) => {
    res.sendFile(path.join(__dirname,'e21public','e21l.html'));
});
...
app.get('/update', (req, res) => {
    if (currentRole !== "Admin") return res.status(403).send("<h1>Access Denied</h1>");
    res.sendFile(path.join(__dirname,'e21public','e21u.html'));
});
...
app.post('/api/update', async (req, res) => {
    ...
    try {
        ...
        res.redirect(`/e21c.html?id=${pid}&name=${encodeURIComponent(details.name)}&model=${encodeURIComponent(details.model)}&price=${details.price}&image=${encodeURIComponent(details.image)}&type=update`);
    } catch (err) {
        console.error("DB ERROR:", err);
        res.status(500).json({ error: err.message });
    } finally {
...
app.get('/delete', (req, res) => {
    if (currentRole !== "Admin") return res.status(403).send("<h1>Access Denied</h1>");
    res.sendFile(path.join(__dirname,'e21public','e21d.html'));
});
...
app.get('/e21bill.html', (req, res) => {
    res.sendFile(path.join(__dirname, 'e21public', 'e21bill.html'));
});
